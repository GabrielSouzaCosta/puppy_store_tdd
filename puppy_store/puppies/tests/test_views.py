...
class UpdateSinglePuppyTest(TestCase):
    def setUp(self):
        self.goiaba = Puppy.objects.create(
            name="Goiaba", age=2, breed="Cat", color="Green"
        )
        self.valid_payload = {
            "name": "Goiaba",
            "age": 3,
    ...
    def test_valid_update_puppy(self):
        response = client.put(
            reverse('get_delete_update_puppy', kwargs={'pk': self.goiaba.pk}),
            data=json.dumps(self.valid_payload),
            content_type='application/json'
        )
...
class DeleteSinglePuppyTest(TestCase):
    ...
    def test_invalid_delete_puppy(self):
        response = client.delete(
            reverse('get_delete_update_puppy', kwargs={'pk' : 999})
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)